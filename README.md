...
**Smoothing.** Live EF uses a **10-minute rolling average** of valid 1-min windows. The displayed drift is hidden until the rolling buffer has at least 5 windows (the first ~5 min post-lock are not yet stable enough to report). If no sample passes the gate for 120 seconds (long coast, stop, or technical descent), the rolling buffer is cleared so the live EF doesn't stitch across non-contiguous efforts. After such a clear, re-display requires the buffer to refill to **8** windows rather than 5: HR lags the resumed power coming out of a recovery, which inflated the first post-gap windows and produced spurious drift spikes up to ~19% — enough to flip a pacing mode. Validated on the 37-ride power library (`tools/eval-rides.py`), the 8-window refill gate roughly halves the worst-case post-gap jump (19% → 11%) at a cost of ~3 fewer readings on stop-start rides.
